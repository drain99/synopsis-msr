...
is associated with a {\tt match} branch, and represents a distinct data constructor,
...
A \sumDtor{} expression $e$ (for an ADT type $T$) must satisfy the following properties:
...
We use the `\addrof{}' operator to extract the address of a memory read expression:
...
\label{sec:eqdef}
Given (1) a \SpecL{} program specification \sprog{}, (2) a C implementation \cprog{},
...
The returned values of \sprog{} and \cprog{} procedures form their observable outputs.
...
The pair $(\pre{},\post{})$ represents the input-output behavior of \cprog{} in terms of the specification \sprog{},
...
However, the loop path \cpath{3,4,5,3} in \cprog{} is still correlated with the path \spath{3,5,3}
in \sprog{} and thus, the product-CFG in \cref{fig:llAllocProductCFG} is indeed well-formed.
