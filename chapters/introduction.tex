...
On the other hand, the C procedure {\tt mk\_list} (defined at line \bpc{4} in \Cref{fig:llAllocC})
...
The \SpecL{} procedure uses the ADT \type{List} to represent a list.
However, the C procedure represents its list using a collection of \type{lnode} objects linked through
...
In order to express equality between these two values (of types \type{List} and \type{i32}) representing lists, we
...
Let us call this lifting constructor \lift{list}{\mem{}}{lnode} and the expression
\lifted{list}{\mem{}}{lnode}{p\ctype{i32}} represents a \type{List} list
constructed from a C pointer $p$ (pointing to a \type{lnode} object) in the memory state \mem{}.
We will formally define \lift{list}{\mem{}}{lnode} in \cref{sec:recrel}.
...
These counterexamples are the backbone of counterexample-guided heuristics for \curved{A1} and \curved{A2} algorithms
as we will see soon.
...
\Cref{fig:diagram} gives an overview of \toolName{} and its interacting components.
\end{itemize}
