...
$S$ and $C$ are {\em equivalent} if for all possible inputs {\tt Input}$_S$ and {\tt Input}$_C$ such that
...
the equality between the \SpecL{} {\tt list} variable {\tt l}$_S$ and the {\tt list} represented by
chasing {\tt lnode} pointers starting at {\tt l}$_C$.
