...
% \vspace{-10px}
\subsection{Results}
% We evaluate \toolName{} on the C implementations discussed above. For functions that
% take two or more data structure values as arguments, we evaluate it for different combinations of
% data layouts for each argument.
% For each program pair, we also \input{chapters/figures/figEvalTable.tex} \vspace{-5px} note the minimum
% under-approximation ($d_u$) and over-approximation ($d_o$) depths at which
% the equivalence proof completed (keeping all other parameters to their default values).
% \Cref{tab:results} lists these program pairs along with the time it took
% to compute equivalence, $d_o$ and $d_u$.
