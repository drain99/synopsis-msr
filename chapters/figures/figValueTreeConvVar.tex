...
\node (out1) [shift={($(scope1.east)+(4mm,-3mm)$)}] {$\xRightarrow[{v_\vtse{1}^\type{Mat}}]{{\tt expand}}$};

\begin{scope}[local bounding box=scope2,shift={($(out1.east)+(15mm,24mm)$)}]
...
\node (out2) [shift={($(scope2.east)+(4mm,-1mm)$)}] {$\xRightarrow[{v_\vtse{2}^\type{Lis}}]{{\tt expand}}$};

\begin{scope}[local bounding box=scope3,shift={($(out2.east)+(18mm,12mm)$)}]
...
\node (out3) [shift={($(scope3.south)+(-2mm,-10mm)$)}] {$\xLeftarrow[{v_\vtse{1}^\type{Mat}}]{{\tt fold}}$};
